...
from nose.tools import assert_equal, assert_is, assert_is_not_none, assert_in
import os.path
...
def test_empty():
    p = Parser()
    assert_is_not_none(type(p))
    assert_is(dict, type(p.to_dict()))


def test_open_file():
    p = Parser(doc=open("test/examples/empty.html"))
    assert_is_not_none(p.__doc__)
    assert_is_not_none(type(p))
    assert_is(dict, type(p.to_dict()))
...
def test_plain_child_microformat():
    ...
    assert_equal(len(result["items"]), 1)
    assert_in("children", result["items"][0])
